...
`scale_colour_manual` takes a named vector (not a list!)

```{r }


vars = unique(both_sexesT$demographic)
map = rep('grey', length(vars))
names(map) = vars
map['kids_all_2534'] = 'black'
map['kids_poor_2534'] = '#00BFC4'



both_sexesT %>% filter(grepl('^(kids_)', demographic)) %>%  ggplot(aes(x=year, y=value, colour=demographic)) + geom_line() + scale_color_manual(values = map)
